            ...
            <tbody>
                <?php if(!empty($rows)) { foreach ($rows as $row) { ?>
                    <tr id="{{$row->id}}">
                        ...
                        <td>{{$row->city->name ?? ""}}</td>
                         <td> 
                            <a data-category="{{$row->category_id}}" data-location="{{$row->location_id}}" data-price="{{$row->price}}" data-id="{{$row->id}}" class="edit" href="javascript:void(0)" data-toggle="modal" data-target="#myModal"> <i class="fa fa-edit"> </i> </a>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form role="form" action="<?php echo route("save-home-budget"); ?>" method="post" enctype="multipart/form-data">
...
<script type="text/javascript">
    ...
    $("body").on("click", ".delete", function () {
        ...
        $.ajax({
            ...
            url: '<?php echo route("delete-home-budget"); ?>',
            data: form_data,
