...
<?php 
      ...
      if(!empty(Auth::user()))
        $getUserFavorite = getUserFavorite(Auth::user()->id);
      $app_setting = getApplicationSettings();
      $currency_symbol = !empty($app_setting["currency_symbol"]) ? $app_setting["currency_symbol"] : "$";
      $search_top_ad = !empty($app_setting["search_top_ad"]) ? $app_setting["search_top_ad"] : "";
      $search_top_ad_link = !empty($app_setting["search_top_ad_link"]) ? $app_setting["search_top_ad_link"] : "";
      $search_top_1 = !empty($app_setting["search_top_1"]) ? $app_setting["search_top_1"] : "";
?>

        <?php 
          
            $imagePath = "storage/images/advertisements/$search_top_ad";
        ?>
        
        @if(!empty($search_top_1) OR  !empty($search_top_ad))
        <div class="col-xl-12 home add-block p-2">
            <div class="container" style="text-align:center">
            <br>
                @if(!empty($search_top_1)) 
                    {!! $search_top_1 !!}
                @else 
                <a href="{{$search_top_ad_link}}" target="_blank"><img src="{{$imagePath}}" alt="Google Ad" /></a>
                @endif
            </div>
        </div>
        @endif
                            ...
                            <div class="accordion" id="accordion4">
                                <div class="accordion-item">
                                    ...
                                    <div id="searchCollapseFour" class="accordion-collapse collapse show" aria-labelledby="searchFour" data-bs-parent="#accordion4">
                                        <div class="accordion-body">
                                            <div class="accordion-inner clearfix">
                                                <form id="search_key_form_location" action="#" method="GET">
                                                    <input class="pr35 form-control" id="autocomplete-ajax" @if(!empty($location)) value="{{$location}}" @endif  placeholder="Location..." type="text" />
                                                    <input class="pr35 form-control" id="selected_location" name="l" @if(!empty($location)) value="{{$location}}" @endif type="hidden" />
                                                    <input class="btn btn-primary refine-go" type="submit" id="searchInputL" value="Go" />
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div class="accordion-inner clearfix">
                                                ...
                                                @foreach ($categories as $category)
                                                   @php  $cSelected = '';
                                                    ...
                                                    @if($childEnd)
                                                    <li> <a href="javascript:void(0);"> <label  data-slug="{{$category->slug}}" for="make_{{$category->id}}" class='title text-link categoryLink'> {{$category->name}} </label><span class='count'><small>{{$category->counter}} </small></span></a></li>
                                                    @else 
...
$("body").on("click" , ".categoryLink" , function() { 
    ...
    var url = "<?php echo url('search'); ?>";
    var url = url + "?c="  + $(this).attr("data-slug");
...
$(document).ready(function() {
  ...
  $('#search_key_form_location').submit(function(event) {
    event.preventDefault();
    $("#skip").val(0);
    searchFunction();
    checkSubAreas();
  ...
  $("body").on("change", ".areaLists" , function() { 
        event.preventDefault();
        $("#skip").val(0);
        searchFunction();
  });

  $("body").on("click", ".filterArea" , function() { 
        event.preventDefault();
        $("#skip").val(0);
        $("#cities-list").modal("hide");
        searchFunction();
  });
  $("body").on("click", ".clearAllAreas" , function() { 
        event.preventDefault();
        $(".areaLists").attr('checked', false);
        $("#skip").val(0);
        $("#cities-list").modal("hide");
        searchFunction();
  });
...
function searchFunction(type ="new") {
   $(".loadings").show();
    ...
    if($("#selected_location").val() != "") { 
        locationsArray.push($("#selected_location").val());
    ...
    $(".areaList:checked").each(function () {
        ...
        if(areaListText == "") { 
            areaListText = areaListText + $(this).attr('data-name');
        } else { 
            areaListText = areaListText + ", "+ $(this).attr('data-name');
        }
        $(".opC").html(areaListText);
  ...
  function checkSubAreas(flag = 0) { 
    ...
    var location_areas = "<?php echo $location_areas; ?>";
   
    if(flag == 0) { 

        if($("#selected_location").val() != "") { 
            locationsIds.push($("#selected_location").val());
        }
...
<div id="cities-list" class="modal fade" role="dialog" >
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-body">
                ...
                <div class="row mt-2">
                    <div class="col-sm-12 col-md-11 col-lg-12">
                        <div class="input-group position-relative d-inline-flex align-items-center">
                            <input type="text" id="search-criteria" name="query" class="form-control input-md" placeholder="Search a location" aria-label="Search a location" value="" autocomplete="off">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="row row-cols-lg-4 row-cols-md-3 row-cols-sm-2 row-cols-1" id="citiesList">
                ...
                <div class="row">
                    <div class="col-12">
                        <ul class="pagination justify-content-center" role="navigation">
                            ...
                            <li class="page-item">
                                <a class="page-link clearAllAreas" rel="next"  aria-label="">Clear</a>
                            </li> &nbsp;&nbsp;&nbsp;
                            <li class="page-item">
                                
                                <a class="page-link btn-primary filterArea" rel="next"  aria-label="">Submit</a>
                            </li>
                        </ul>
                    </div>
                </div>
    ...
    $(function() { 
        ...
        $('#autocomplete-ajax').autocomplete({
            ...
            onSelect: function (suggestion) {
                $("#selected_location").val(suggestion.data);
                // $("#skip").val(0);
                // searchFunction();
                // checkSubAreas();
